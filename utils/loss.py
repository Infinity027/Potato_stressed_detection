...
class YoloLoss(nn.Module):
    def __init__(self, S=7, C=2, label_smooth=0.0):
    ...
    def forward(self, preds:torch.tensor, targets:list):
        ...
        #create target shape: [batch_size, S*S, C + 5]
        targets = self.batch_build_target(targets)

        #predictions shape: [batch_size, S*S, C + B*5] with B=1.
        # Split predictions: first C are class predictions, we have [conf, x, y, w, h]
        pred_cls = preds[..., :self.C].permute(0,2,1)
        pred_boxes = preds[..., self.C:]

        # Build target components.
        target_obj = (targets[..., 1] == 1).float()        #only conatins True and False values [N, S*S]
        target_noobj = (targets[..., 1] == 0).float()       #only conatins True and False values
        target_cls = targets[..., 0].long()                # [N, S*S]
        
        with torch.no_grad():
            iou = self.calculate_iou(pred_boxes[..., 1:], targets[..., 2:])

        # ======= Object loss [only for the responsible box] =======
        obj_loss = self.mse(pred_boxes[..., 0], iou) * target_obj
        obj_loss = obj_loss.sum() / self.batch_size

        # ======= No Object loss [penalize the other box's confidence] =======
        noobj_loss_b1 = self.mse(pred_boxes[..., 0], targets[..., 1]*0) * target_noobj
        noobj_loss = noobj_loss_b1.sum()/self.batch_size

        # ======= box loss =======
        # Box (localization) loss: converting width and height sqrt.
        pred_wh = torch.sqrt(pred_boxes[..., 3:])
        targets_wh = torch.sqrt(targets[..., 4:])
        box_loss_xy = self.mse(pred_boxes[..., 1:3], targets[..., 2:4]).sum(dim=-1)
    ...
    def transform_cxcywh_to_x1y1x2y2(self, boxes):
        xc = (boxes[..., 0] + self.grid_x) / self.S
        yc = (boxes[..., 1] + self.grid_y) / self.S
        x1 = xc - boxes[..., 2] / 2
        y1 = yc - boxes[..., 3] / 2
        x2 = xc + boxes[..., 2] / 2
        y2 = yc + boxes[..., 3] / 2
...
if __name__ == "__main__":
    loss = YoloLoss()
    preds = torch.rand(4,49,7)
    targets = torch.rand(4,49,6)
    total_loss, obj_loss, noobj_loss, box_loss, cls_loss = loss(preds=preds,targets=targets)
    print("Totol loss:", total_loss)
    print("box loss:", box_loss)
    print("obj loss:", obj_loss)
    print("noobj loss:", obj_loss)
